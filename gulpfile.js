...
gulp.task('dev-pug-compile', function() {
  return (
    gulp
      .src([`./${path.root}/${path.src}/**/*.+(pug|jade)`], {
...
gulp.task('dev-bootstrap-scss-compile', function() {
  return (
    gulp
      .src(
        [
          `./${path.root}/${path.src}/assets/bootstrap/scss/**/*.scss`,
          `!./${path.root}/${path.src}/assets/bootstrap/scss/bootstrap-grid.scss`,
          `!./${path.root}/${path.src}/assets/bootstrap/scss/bootstrap-reboot.scss`,
        ],
        {
      ...
      .pipe(sass())
      .pipe(sourcemaps.write('./'))
      .pipe(gulp.dest(`./${path.root}/${path.src}/assets/bootstrap/css`))
  );
});
...
gulp.task('dev-vendors-scss-compile', function() {
  return (
    gulp
      .src([`./${path.root}/${path.src}/assets/vendors/source/**/*.scss`], {
...
// === удалить содержимое папки prod
gulp.task('prod-delete', function() {
  return del([`./${path.root}/${path.prod}/**`], { read: false });
});
...
// удаление файла bootstrap без вендорных префиксов в режиме production
gulp.task('prod-del-bootstrap-without-prefix', function() {
  return del(
    [`./${path.root}/${path.prod}/assets/bootstrap/css/bootstrap.css`],
    { read: false }
  );
});
...
gulp.task('dev-watch', function() {
  ...
  // при изменении исходников bootstrap
  // компиляция
  gulp.watch(
    [`./${path.root}/${path.src}/assets/bootstrap/scss/**/*.scss`],
...
gulp.task(
  ...
  gulp.series([
    ...
    'prod-del-bootstrap-without-prefix',
